    ...
    @IBAction func segmentControlPressed(_ sender: UISegmentedControl)
    {
        ...
        switch sender.selectedSegmentIndex
        {
            case 1: //衛星
                ...
                //定義攝影機的座標位置
                let eyeFrom = CLLocationCoordinate2D(latitude: CLLocationDegrees(lblLatitude.text!)!, longitude: CLLocationDegrees(lblLongitude.text!)!)
                //定義攝影位置與高度（視角）
            ...
            case 3: //3D（必須實機測試）
                ...
                let ground = CLLocationCoordinate2D(latitude: CLLocationDegrees(lblLatitude.text!)!, longitude: CLLocationDegrees(lblLongitude.text!)!)
                //定義攝影機的座標位置
                let eyeFrom = CLLocationCoordinate2D(latitude: CLLocationDegrees(lblLatitude.text!)!+0.005, longitude: CLLocationDegrees(lblLongitude.text!)!)
                //定義攝影位置與高度（視角）
                let camera = MKMapCamera(lookingAtCenter: ground, fromEyeCoordinate: eyeFrom, eyeAltitude: 30)
                //改變地圖以3D方式呈現
                myMap.mapType = .satelliteFlyover
                //允許可以從斜角方向觀看地圖（false只能從正上方）
                myMap.isPitchEnabled = true
                //在地圖上加上攝影機
                myMap.camera = camera
            default:    //預設標準模式，不設定攝影機
                //改變地圖以標準模式呈現
...
//            print("解碼資訊：\(String(describing: placemarks?[0].addressDictionary))")
//            if let dicAddress = placemarks?[0].addressDictionary
//            {
//                for (key,value) in dicAddress.enumerated()
//                {
//                    print("\(key):\(value)")
//                }
//            }
