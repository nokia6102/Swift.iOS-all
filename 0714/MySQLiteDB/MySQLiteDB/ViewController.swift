...
class ViewController: UIViewController,UIPickerViewDelegate,UIPickerViewDataSource,UIImagePickerControllerDelegate,UINavigationControllerDelegate
{
    ...
    @IBAction func btnInsert(_ sender: UIButton)
    {
        ...
        if db != nil
        {
            ...
            if sqlite3_step(statement) == SQLITE_DONE
            {
                ...
                alert.addAction(UIAlertAction(title: "確定", style: .default, handler: nil))
                present(alert, animated: true, completion: nil)
                //重新查詢一次（按一下查詢按鈕，取得離線資料集arrTable）
                btnQuery(nil)
            }
            else
            {
    ...
    @IBAction func btnUpdate(_ sender: UIButton)
    {
        ...
        if db != nil
        {
            ...
            let imageData = UIImageJPEGRepresentation(imgPicture.image!, 0.8)! as NSData
            //準備SQL的更新指令
            let sql = String(format: "update student set name='%@',gender=%li,picture=?,phone='%@',address='%@',email='%@',class='%@' where stu_no = '%@'", txtName.text!,pkvGender.selectedRow(inComponent: 0),txtPhone.text!,txtAdress.text!,txtEmail.text!,arrClass[pkvClass.selectedRow(inComponent: 0)], txtNo.text!)
            print("修改指令：\(sql)")
            //把SQL指令轉成C語言字串
            let cSql = sql.cString(using: .utf8)
            //宣告儲存執行結果的變數
    ...
    @IBAction func btnDelete(_ sender: UIButton)
    {
        ...
        if txtNo.text == ""
        {
            let alert = UIAlertController(title: "輸入訊息錯誤", message: "請提供學號，才能刪除資料！", preferredStyle: .alert)
            alert.addAction(UIAlertAction(title: "確定", style: .default, handler: nil))
            present(alert, animated: true, completion: nil)
            return  //直接離開函式
        }
        ...
        {
            //準備SQL的刪除指令
            let sql = String(format: "delete from student where stu_no='%@'", txtNo.text!)
            print("刪除指令：\(sql)")
            //把SQL指令轉成C語言字串
            let cSql = sql.cString(using: .utf8)
            //宣告儲存執行結果的變數
